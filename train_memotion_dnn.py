...
def train(config: dict):
    ...
    if len(config["image_models"]) > 0:
        ...
        valid_image_data = np.load(config['base_dir'] + "/resources/Memotion7k/image_predictions/valid_image_predictions.npy")
        test_image_data = np.load(config['base_dir'] + "/resources/Memotion7k/image_predictions/test_image_predictions.npy")
        train_image_data = np.load(config['base_dir'] + "/resources/Memotion7k/image_predictions/train_over_sampled_image_predictions.npy")
    ...
    results_dir_path = "memotion_dnn_results"
    ...
    if file_utils.path_exists(config["base_dir"] + '/' + results_dir_path + "/" + config['result_file_name']):
        print('Writing the results to: '+ config["base_dir"] + '/' + results_dir_path + "/" + config['result_file_name'])
...
if __name__ == "__main__":
    argv = (sys.argv[1:])
    config_path = 'memotion_dnn_config.json'
    try:
